...
try:  # Support both package and flat execution contexts
    from backend.config_manager import (  # type: ignore
...
class ExportImportMixin:
    """Mixin class for handling export/import operations."""
        ...
        def task():
            ...
            if override:
                try:
                    ...
                    pass
            success, msg = self.processor.export_m3u(
                self.channels,
    ...
    def export_selection_clicked(self, _):
        """Export current session data to backup file."""

        def task():
            try:
                self.update_status("Exporting session...")
                timestamp = int(time.time())
                backup_file = get_output_path(
                    self.processor.config, "session_backups", str(timestamp)
                )
    ...
    def import_selection_clicked(self, _):
        """Import session data from most recent backup file."""

        def task():
            try:
                self.update_status("Looking for session backup...")
                backup_dir = get_output_path(
                    self.processor.config, "session_backups", ""
                ).rsplit(os.sep, 1)[0]
                backups = glob.glob(
                    os.path.join(backup_dir, "session_backup_*.json")
                ) or glob.glob("data/session_backup_*.json")
                if not backups:
                ...
                self.playlist_sources = data.get("playlist_sources", [])
                self.stream_urls_seen = set(data.get("stream_urls_seen", []))
                if self.channels:
                ...
                msg = f"Session imported from {os.path.basename(latest)}: {len(self.channels)} channels"
                self.update_status(msg)
                self.page.snack_bar = self._snack(msg)
                self.page.update()
            except Exception as e:
    ...
    def export_channel_list_clicked(self, _):
        """Export list of selected channels to JSON file."""
        ...
        def task():
            try:
                ...
                # Export only selected channels
                selected_channels = [
                    ch for ch in self.channels if ch.get("selected", False)
                ]
                if not selected_channels:
                    self.update_status("No channels selected for export", is_error=True)
                    return

                self.update_status("Exporting selected channel list...")
                channel_list = [
                    {"name": ch.get("name", ""), "selected": True}
                    for ch in selected_channels
                    if ch.get("name")
                ]
                timestamp = int(time.time())
                export_file = get_output_path(
                    self.processor.config, "channel_lists", str(timestamp)
                )
                ...
                msg = (
                    f"Channel list exported to {os.path.basename(export_file)} "
                    f"({len(channel_list)} selected channels)"
                )
                self.update_status(msg)
                self.page.snack_bar = self._snack(msg)
                self.page.update()
    ...
    def import_channel_list_clicked(self, _):
        """Open file picker to import channel selection list."""
        if not self.channels:
            self.update_status(
                "Load a playlist before importing channel list", is_error=True
            )
            return
        self.update_status(
            "Select a channel list JSON file to import (will auto-optimize first)..."
        )
        try:
            if not hasattr(self, "channel_list_picker"):
                import flet as ft

                self.channel_list_picker = ft.FilePicker(
                    on_result=self.on_channel_list_file_picked
                )
                self.page.overlay.append(self.channel_list_picker)
            self.channel_list_picker.pick_files(
    ...
    def on_channel_list_file_picked(self, e):
        """Handle channel list file selection result."""
        if not e.files:
        ...
        if not path or not os.path.isfile(path):
            ...
            if content_bytes:
                ...
                self.update_status(
                    "Picked file has no accessible path/content", is_error=True
                )
                return
    ...
    def _start_channel_list_import(self, using_file: str):
        """Import and apply channel selections from file."""
        ...
        def import_task():
            try:
                ...
                for ch in imported_channels:
                    ...
                    if name:
                        imported_map[self.normalize_channel_name(name)] = ch.get(
                            "selected", False
                        )
                self.update_status(
                    "Optimizing playlist before applying selections (merge+clean+dead)..."
                )
                try:
                    q_removed, u_removed, d_removed = self._optimize_all_core_internal()
                    self.update_status(
                        f"Optimization done: {q_removed} quality + {u_removed} unwanted + "
                        f"{d_removed} dead removed; applying selections..."
                    )
                except Exception as op_ex:
                    self.update_status(
                        f"Optimization failed (continuing import): {op_ex}",
                        is_error=True,
                    )
                matches = 0
                ...
                self.save_current_session()
                msg = (
                    f"Import complete: applied selections from "
                    f"'{os.path.basename(using_file)}' to {matches} channels"
                )
                self.update_status(msg)
                self.page.snack_bar = self._snack(msg)
                self.page.update()
        ...
        run_background(
            ...
            before=lambda: self.show_progress(True),
            after=lambda: self.show_progress(False),
            name="import-channel-list",
        )

    def _snack(self, msg: str):
        """Create a success snackbar with consistent styling."""
        import flet as ft
