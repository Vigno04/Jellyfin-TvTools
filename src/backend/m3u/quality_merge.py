...
def merge_quality(channels: List[Dict[str, Any]], quality_cfg: Dict[str, Any], *, progress_callback: ProgressCb = None) -> Tuple[List[Dict[str, Any]], int]:
    ...
    for ch in channels:
        base = qm.base_channel_name(ch['name']); ch['_base_name'] = base
    ...
    for _, group in single_groups.items():
        ch = group[0]
        if quality_cfg.get('normalize_channel_names', True) and ch['name'] != ch['_base_name']:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for base, group in multi_groups.items():
            for ch in group:
                url = ch['lines'][-1]; fut = url_future.get(url)
                if fut is None:
                    fut = ex.submit(checker.analyze, url, None)
                    url_future[url] = fut
                work.append((base, ch, fut))
        completed = 0; total_futs = len(work)
        for base, ch, fut in work:
            metrics = fut.result().as_dict(); ch['quality_metrics'] = metrics
            ch['_quality_score'] = metrics.get('score') or 0; ch['_priority_index'] = qm.quality_priority(ch['name'])
            completed += 1
    ...
    for _, group in multi_groups.items():
        ...
        merged.append(best); removed_count += len(group) - 1
...
def _merge_channel_attributes(best_channel: Dict[str, Any], all_variants: List[Dict[str, Any]]):
    ...
    for attr in mergeable:
        if not best_channel.get(attr):
            for variant in all_variants[1:]:
                if variant.get(attr):
                    best_channel[attr] = variant[attr]; break
